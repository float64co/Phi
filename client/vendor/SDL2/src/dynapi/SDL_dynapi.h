...
#elif defined(PHI_SDL2_JOYSTICK_ONLY)
/* Phi-specific (see VENDORED.md in this vendor tree): Phi only vendors
 * SDL2's joystick+gamecontroller subsystem, deliberately without video/
 * audio/render -- the dynamic-API dispatch table (this file's whole
 * point) statically references EVERY public SDL2 function including
 * ones from the subsystems Phi doesn't compile in, which makes the
 * dispatch table fail to link unless those subsystems are present too.
 * Turning it off removes that indirection layer entirely, so only
 * symbols Phi's own code actually calls need to resolve. This is the
 * SAME tradeoff the platforms above already accept (no in-field SDL
 * update without recompiling) -- consistent with how Bullet is already
 * vendored in this project: a pinned, static, non-dynamically-updated
 * snapshot, not a new risk pattern. PHI_SDL2_JOYSTICK_ONLY is defined by
 * Phi's own Makefile (see COMMON_SRCS's SDL2 section), not by upstream
 * SDL2 itself. */
#define SDL_DYNAMIC_API 0
#endif
