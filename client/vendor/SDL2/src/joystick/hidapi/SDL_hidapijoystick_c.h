...
/* Phi-specific: NOT defined (see VENDORED.md) -- SDL_hidapi_steam.c/
 * SDL_hidapi_steamdeck.c both #include "steam/controller_constants.h",
 * which genuinely does not exist anywhere in the official SDL2-2.30.0
 * release tarball (confirmed by direct inspection, not assumed -- a real
 * upstream packaging gap in that specific release, not a Phi omission).
 * The Steam Deck's built-in controller and other HID-only Steam
 * Controllers still work through the ordinary Linux joystick/evdev path
 * + SDL_GameControllerDB mapping below; only this HIDAPI-specific fast
 * path (lower latency, gyro/haptics access) is unavailable.
